...
// write code here
let dataArr = json.data.split(',');
let keyValues = [];
for (let i = 0; i < dataArr.length; i++) {
    if (i !== 0 && i % 2 === 1) {
        let ageValue = parseInt(dataArr[i].substring(dataArr[i].indexOf("=")+1))
        let keyValue = dataArr[i-1].substring(dataArr[i-1].indexOf("=")+1);
        if (ageValue === 32) {
...
if (keyValues.length > 0) {
    let fileStream = fs.createWriteStream('./npm-tests/output.txt', {
        flags: 'a'
    });
